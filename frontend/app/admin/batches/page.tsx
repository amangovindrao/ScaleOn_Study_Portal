...
export default function BatchesPage() {
  ...
  async function handleDelete(id: string) {
    ...
    const res = await api.delete(`/catalog/batches/${id}`);
    if (!res.success) {
            ...
            <div key={batch.id} className="bg-white border border-slate-200/80 rounded-2xl p-5 space-y-3 shadow-sm hover:shadow-md transition">
              <div className="flex items-start justify-between">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <button onClick={() => handleDelete(batch.id)} className="text-red-500 hover:text-red-700 text-[11px] font-medium flex items-center gap-1 transition-colors px-1">
